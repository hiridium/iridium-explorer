<?php
	...
	if (@$_POST['issueasset']) {
		...
		$success=no_displayed_error_result($issuetxid, iridiun('createrawsendfrom', $_POST['from'], $addresses, $datas, 'send'));
	...
	if (no_displayed_error_result($getaddresses, iridiun('getaddresses', true))) {
		...
		if (no_displayed_error_result($listpermissions,
			iridiun('listpermissions', 'issue', implode(',', array_get_column($getaddresses, 'address')))
		))
