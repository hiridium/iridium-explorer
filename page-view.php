<?php
	...
	$labels=iridiun_labels();

	no_displayed_error_result($liststreams, iridiun('liststreams', '*', true));
	no_displayed_error_result($getinfo, iridiun('getinfo'));
	...
	foreach ($liststreams as $stream) {
		if (@$_POST['subscribe_'.$stream['createtxid']])
			if (no_displayed_error_result($result, iridiun('subscribe', $stream['createtxid']))) {
...
<?php
	...
	if (isset($viewstream)) {
		if (isset($_GET['key'])) {
			$success=no_displayed_error_result($items, iridiun('liststreamkeyitems', $viewstream['createtxid'], $_GET['key'], true, const_max_retrieve_items));
			$success=$success && no_displayed_error_result($keysinfo, iridiun('liststreamkeys', $viewstream['createtxid'], $_GET['key']));
			$countitems=$keysinfo[0]['items'];
			$suffix=' with key: '.$_GET['key'];
			
		} elseif (isset($_GET['publisher'])) {
			$success=no_displayed_error_result($items, iridiun('liststreampublisheritems', $viewstream['createtxid'], $_GET['publisher'], true, const_max_retrieve_items));
			$success=$success && no_displayed_error_result($publishersinfo, iridiun('liststreampublishers', $viewstream['createtxid'], $_GET['publisher']));
			$countitems=$publishersinfo[0]['items'];
			$suffix=' with publisher: '.$_GET['publisher'];
		
		} else {
			$success=no_displayed_error_result($items, iridiun('liststreamitems', $viewstream['createtxid'], true, const_max_retrieve_items));
...
<?php
			...
			foreach ($items as $item) {
				$oneoutput=true;
				$keys=isset($item['keys']) ? $item['keys'] : array($item['key']); // support Iridium 2.0 or 1.0
...
?>
				...
				} elseif ( is_array($item['data']) && array_key_exists('json', $item['data']) ) { // Iridium 2.0 JSON item
					$showlabel='JSON data';
				...
				} elseif ( is_array($item['data']) && array_key_exists('text', $item['data']) ) { // Iridium 2.0 text item
					$showlabel='Text data';
					$showhtml=html($item['data']['text']);
					
				} else { // binary item
					if (is_array($item['data'])) { // long binary data item
						if (no_displayed_error_result($txoutdata, iridiun('gettxoutdata', $item['data']['txid'], $item['data']['vout'], 1024))) // get prefix only for file name
