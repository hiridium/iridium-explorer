<?php
	if (@$_POST['getnewaddress'])
		no_displayed_error_result($getnewaddress, iridiun('getnewaddress'));
?>
...
<?php
	if (no_displayed_error_result($peerinfo, iridiun('getpeerinfo'))) {
?>
...
<?php
	if (no_displayed_error_result($getaddresses, iridiun('getaddresses', true))) {
		...
		if (no_displayed_error_result($listpermissions,
			iridiun('listpermissions', 'all', implode(',', array_keys($addressmine)))
		))
			foreach ($listpermissions as $listpermission)
				$addresspermissions[$listpermission['address']][$listpermission['type']]=true;
		
		no_displayed_error_result($getmultibalances, iridiun('getmultibalances', array(), array(), 0, true));
