<?php
	...
	if (@$_POST['approvetxfilter'] || @$_POST['disapprovetxfilter']) {
		if (no_displayed_error_result($approvetxid, iridiun(
	...
	if (@$_GET['txfilter']) {
		if (no_displayed_error_result($listtxfilters, iridiun(
			'listtxfilters', $_GET['txfilter'], true
		)))
			$txfilter=$listtxfilters[0];
			
		no_displayed_error_result($filtercode, iridiun(
			'getfiltercode', $_GET['txfilter']
		));
	}
	
	if (@$_GET['streamfilter']) {
		if (no_displayed_error_result($liststreamfilters, iridiun(
		...
		no_displayed_error_result($liststreams, iridiun(
			'liststreams', '*', true
		));
	}
