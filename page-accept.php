<?php
	if (@$_POST['unlockoutputs'])
		if (no_displayed_error_result($result, iridiun('lockunspent', true)))
	...
	if (@$_POST['decodeoffer'] || @$_POST['completeoffer']) {
		if (no_displayed_error_result($decoded, iridiun('decoderawexchange', $_POST['hex']))) {

			if (@$_POST['completeoffer']) {
				if (no_displayed_error_result($prepare, iridiun('preparelockunspentfrom', $_POST['from'], ask_offer_to_assets($decoded['ask'])))) {
						// output_success_text('Exchange successfully prepared using transaction '.$prepare['txid']);
			
					if (no_displayed_error_result($rawexchange, iridiun('appendrawexchange', $_POST['hex'],
						$prepare['txid'], $prepare['vout'], ask_offer_to_assets($decoded['offer'])))) {
			
						if (no_displayed_error_result($sendtxid, iridiun('sendrawtransaction', $rawexchange['hex'])))
...
<?php
	...
	if (no_displayed_error_result($getaddresses, iridiun('getaddresses', true))) {
		...
		if (no_displayed_error_result($listassets, iridiun('listassets')))
			$allassets=array_get_column($listassets, 'name');

		foreach ($sendaddresses as $address) {
			if (no_displayed_error_result($allbalances, iridiun('getaddressbalances', $address, 0, true))) {
				
				if (count($allbalances)) {
					$assetunlocked=array();

					if (no_displayed_error_result($unlockedbalances, iridiun('getaddressbalances', $address, 0, false))) {
