<?php
	...
	if (@$_POST['testtxfiltercode']) {
		if (no_displayed_error_result($testtxfilter, iridiun('testtxfilter', false, $_POST['code']))) {
	...
	if (@$_POST['createtxfilter']) {
		$success=no_displayed_error_result($createtxid, iridiun('createfrom',
	...
	if (@$_POST['testtxfiltersend'])
		if (no_displayed_error_result($createrawsendfrom, iridiun('createrawsendfrom',
	...
	if (isset($sendrawtx)) {
		if (no_displayed_error_result($testtxfilter, iridiun_with_raw(
	...
	if (no_displayed_error_result($getaddresses, iridiun('getaddresses', true))) {
		...
		if (no_displayed_error_result($listpermissions,
			iridiun('listpermissions', 'send', implode(',', array_get_column($getaddresses, 'address')))
		))
			$sendaddresses=array_get_column($listpermissions, 'address');
			
		if (no_displayed_error_result($listpermissions, iridiun('listpermissions', 'receive')))
			$receiveaddresses=array_get_column($listpermissions, 'address');
		
		if (no_displayed_error_result($listpermissions,
			iridiun('listpermissions', 'admin', implode(',', array_get_column($getaddresses, 'address')))
		))
			$adminaddresses=array_unique(array_get_column($listpermissions, 'address'));

		if (no_displayed_error_result($listpermissions,
			iridiun('listpermissions', 'create', implode(',', $adminaddresses))
		))
			$admincreateaddresses=array_unique(array_get_column($listpermissions, 'address'));
	}
	
	no_displayed_error_result($listassets, iridiun('listassets'));
	no_displayed_error_result($liststreams, iridiun('liststreams'));

	$getinfo=iridiun_getinfo();
	
	$usableassets=array();
	if (no_displayed_error_result($gettotalbalances, iridiun('gettotalbalances')))
...
<?php
	if (no_displayed_error_result($listtxfilters, iridiun('listtxfilters', '*', true))) {
...
?>						
						...
						<div class="form-group">
							...
							<div class="col-sm-9">
								...
								<span id="helpBlock" class="help-block">Raw transactions can be created using the <code>iridiun-cli</code> command line tool and the <code>createrawsendfrom</code> or <code>createrawtransaction</code> command.</span>
							</div>
						</div>
						<div class="form-group">
							<div class="col-sm-offset-3 col-sm-9">
