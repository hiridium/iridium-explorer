<?php
	...
	if (iridiun_has_custom_permissions())
	...
	if (@$_POST['grantrevoke']) {
		...
		if ($_POST['operation']=='grant')
			$success=no_displayed_error_result($permissiontxid, iridiun('grantfrom',
				$_POST['from'], $_POST['to'], implode(',', $permissions)));

		elseif ($_POST['operation']=='revoke')
			$success=no_displayed_error_result($permissiontxid, iridiun('revokefrom',
				$_POST['from'], $_POST['to'], implode(',', $permissions)));
	...
	if (no_displayed_error_result($getaddresses, iridiun('getaddresses', true))) {
		...
		if (no_displayed_error_result($listpermissions,
			iridiun('listpermissions', 'admin,activate', implode(',', array_get_column($getaddresses, 'address')))
		))
...
<?php
	if (no_displayed_error_result($listpermissions, iridiun('listpermissions'))) {
