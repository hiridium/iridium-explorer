<?php
	$max_upload_size=iridiun_max_data_size()-512; // take off space for file name and mime type
	$allow_multi_keys=iridiun_has_multi_item_keys();
	...
	if (@$_POST['publish']) {
		...
		} elseif (iridiun_has_json_text_items()) { // use native JSON and text objects in Iridium 2.0
		...
		} else // convert entered text to binary for Iridium 1.0
		...
		if ($_POST['offchain']) // need to separate cases here since Iridium 1.0 publishfrom API has no 'options' parameter
			$result=iridiun('publishfrom', $_POST['from'], $_POST['name'], $keys, $data, 'offchain');
		else
			$result=iridiun('publishfrom', $_POST['from'], $_POST['name'], $keys, $data);
...
<?php
	if (iridiun_has_off_chain_items()) {
?>
...
<?php
	if (iridiun_has_json_text_items()) {
?>
